...
`gf.MessagePassing` 是编写图程序的主要方式：声明沿每条边传递什么、
消息如何组合；编译器负责生成关系遍历、并行调度以及反向模式 [VJP](https://en.wikipedia.org/wiki/Automatic_differentiation)。

![MessagePassing 数据流：字段按边收集，经边 UDF 变换，由 reducer 合并，由节点 UDF finalize；VJP 由编译器生成](assets/message-passing-flow.svg)
...
无需手写任何反向函数 —— `gf.autograd.grad` 直接使用编译器生成的 VJP。

## 用法模式 { #usage-patterns }

以下列出 `MessagePassing` 支持的所有用法，每种都给出语义公式和
可运行示例。记号约定：$e = (j \to i)$ 表示一条边，$m_e$ 是它的消息，
结果按目标节点 $i$ 给出。
...
**分布式 [halo](https://en.wikipedia.org/wiki/Halo_(computer_science)) 交换。** $\text{out}^{(r)}_i$ 在分区关系上按 rank 本地计算；[distributed_halo.py](examples/distributed-memory.md#two-process-halo-exchange)。

## Subclass 契约 { #the-subclass-contract }

- `reducer` —— 类属性（默认 `gf.sum()`），也可以在 `__init__` 里按实例
  设置。任何 `gf.Reducer` 都可以；见 [Reducers](#reducers)。
...
调用仅限关键字参数，返回一个形状为 `(num_dst, *feature_shape)` 的
`gf.Tensor`：
...
- 原生路径：字段值必须是 `gf.Tensor`，首维分别为 `num_src` /
  `num_dst` / `num_edges`，且位于图所在设备上。
- 如果任一值是 Torch 张量，调用会走可选的 Torch 互操作适配器
  （零拷贝 `from_torch` 适配）。
- 首次调用完成捕获、特化、lowering 并缓存；之后结构相同的调用复用
  已编译的变体。没有单独的编译步骤。
...
知道一点：流式 reducer 是*在 `edge` 内部调用的*，并返回一个 staged 条目。
...
用户自定义 reducer 继承 `gf.Reducer`，可以携带元组状态 —— 均值就是
`(sum, count)`，见可运行的
...
| 可融合的 reducer | `gf.sum()` → 边中心 tile（本节）；`gf.online_softmax()` → GAT 风格注意力，其中 nn 为每条边产出一个标量分数、`value` 是字段 gather，编译为以行为中心的 online-softmax tile kernel，含前向和融合反向（[示例](examples/attention.md#gat-edge-attention-with-an-nn-score)） |
| Launch geometry | 是调优旋钮而非语义：`gf.nn.trace(mlp, block_e=256, num_warps=8)` 设置 tile 大小（2 的幂，16–1024）。默认值按 lowering 区分：sum tile 为 128/4 —— 在 400 万边半径工作负载上扫描得到的最优点（2.9 ms 前向+反向，对比 64 时的 3.3 ms 和 256 时的 3.5 ms）—— 注意力 tile 为 16/1，在典型注意力度数下小块浪费的 lane 更少 |
| 不支持的结构 | 在 `gf.nn.trace` 时以 `NotImplementedError` 拒绝：`BatchNorm`（统计量跨整个边批次计算 —— 天然跨边）、dropout（重算式 VJP 无法重放的随机状态）、分支/残差（超出已证明的线性链结构）；超出运行时支持范围（reducer、dtype、设备）的调用回退到精确的 eager oracle |
...
| 构造器 | Torch 张量调用 | 编译后的 CUDA 特化 | 原生 `gf.Tensor`（自动生成 VJP） |
|---|---|---|---|
...
eager oracle 永远可用，本身就是语义的定义；编译特化必须与之精确一致。
没有任何特化能处理的调用照样能跑 —— 走 oracle，结果和 autograd
都正确。
