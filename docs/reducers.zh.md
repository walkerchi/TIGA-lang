...
前置内容：[编程模型](programming-model.md)。
[reducer](https://en.wikipedia.org/wiki/Fold_(higher-order_function))
指定同一目标节点收到的消息如何合并。先选择内置实现；
只有内置实现无法表达计算时，才需要阅读后面的自定义代数。

## 选择一个内置 reducer { #choose-reducer }

| 需要的结果 | 选择 | 注意事项 |
|---|---|---|
| 消息之和 | `tg.sum()` | 默认空行结果为 0 |
| 消息均值 | `tg.mean()` | 空行结果为 NaN，不自动变成 0 |
| 消息乘积 | `tg.prod()` | 空行结果为 1 |
| 按 score 加权的平均 | `tg.online_softmax()` | 在 `edge()` 内传入 score 和 value；执行范围有额外限制 |

例如，在已有 MessagePassing 写法中选择均值：
...
图与字段的绑定方式不变。完整调用见
[自定义邻居均值示例](examples/message-passing.md#user-defined-reducer)；
内置签名见 [API 参考](api.md#reducers)。
图、dtype 与执行路径仍需符合[支持矩阵](roadmap.md#feature-support)。

## 进阶：reducer 的内部表示 { #reducer-representation }

以下内容解释自定义 reducer、代数契约和编译方式，不是使用内置 reducer 的前置要求。
...
**`tg.sum(identity=…)`。** 加法[幺半群](https://baike.baidu.com/item/幺半群)，直接 lowering 成段归约求和；
...
**`tg.mean()`。** 邻居均值，用内置元组状态 `(sum, count)`
实现；[见下文](#gfmean)。
...
**`tg.prod()`。** 乘积幺半群，提升为零安全的乘积 IR；[见下文](#gfprod)。

$$
\text{out}_i = \prod_e m_e
$$

**`tg.online_softmax()`。** 把 flash attention 写成元组状态
...
**手写元组状态版的均值。** 把 `tg.mean()` 内置背后的代数显式写成
自定义 reducer；[custom_reducer.py](examples/message-passing.md#user-defined-reducer)。

**单遍矩统计。** 用 `(count, sum, sum_of_squares)` 一遍算出[方差](https://baike.baidu.com/item/方差)，已证明
逐分量可加，并带有生成的 VJP；[见下文](#neighbor-variance-in-one-pass)。

**手写乘积幺半群。** 把 `tg.prod()` 内置背后的代数显式写出来，用于
...
**用 `tg.sum()` 聚合 torch.nn 产生的消息。** 消息是一个融合、可训练的
...
**用 `tg.online_softmax()` 聚合 torch.nn 产生的分数。** GAT 风格的
...
状态可以是标量或元组。邻居均值用 `lift` 记录值和计数，`combine` 分别
相加，`finalize` 再用总和除以计数。见可运行的
...
| `associative` | 分组方式不改变代数结果 | **原生并行 lowering 要求 `True`**；永远不会从 Python 源码推导——声明即契约 |
| `commutative` | 交换操作数不改变代数结果 | 允许 combine 树重排操作数 |
...
普通单消息 `edge()` 直接 `return src.value`，类属性 `reducer` 已经指定聚合规则；
不需要再写 `self.reducer(...)`。尤其内置 `tg.sum()` 应直接返回 Tensor。
下面是自定义 reducer 的显式绑定形式；online-softmax 的多输入接口也采用类似形式。
它只把输入交给已声明的 reducer，返回一个 **staged 绑定**，不会在单条边上执行归约：
...
- 多个消息被打包为 reducer 的输入，再由 `lift` 构造内部状态；消息元组不等于状态元组：
...
其中 `m = max(m1, m2)`。合并前先把两个部分状态重新缩放到
同一范围，结果是 value 的 softmax 加权均值。重新缩放发生在
...
一个 `tg.nn.trace` 过的网络（GAT 风格的学习注意力）：在 CUDA 上它会
...
**为什么没有 `max` / `min` / `median` 内置？** max 的代数很简单，
但原生可微 op 集合尚无带次梯度 VJP 的 `maximum` op。
`tg.online_softmax()` 计算平滑的加权均值，不等价于 max，不能静默替代。median 和
分位数则是本质上的另一回事：**不是结合幺半群**——不存在固定
大小的状态 `combine(s, s')` 能累积出 median——从根本上不属于
单遍 reducer 语义，需要两遍算法或 sketch（t-digest、矩）。

## 让 nn 产出消息或 score { #neural-networks-in-the-message-or-the-score }

一个 `tg.nn.trace` 过的 `torch.nn` 模块可以在两个点融合进归约——
两者都编译为单个融合的 CUDA kernel，逐边激活永远不会离开 tile，
且两者都能端到端训练（字段、位置以及所有模块权重都会收到生成的
梯度）：

**nn 产生消息**，由 `tg.sum()` 组合——编译为以边为中心的 tile kernel
...
**nn 产生分数**，由 `tg.online_softmax()` 组合——GAT 风格的学习
...
一次遍历累积矩 `(count, sum, sum_of_squares)`，用于需要邻域统计的
特征归一化层。finalize 计算方差：
...
可靠性风格的聚合取所有入边概率的乘积，即进入 `i` 的*每个*
gate 都保持打开的概率。这正是 `tg.prod()` 内置所用的代数——这里手写
一遍，展示其中的机制：
...
`tg.online_softmax()` 并没有什么魔法——把同一个代数写成用户 reducer，
...
1. `tg.sum()` → 直接段归约求和；nn 子图消息进一步 lowering 为以边
   为中心的融合 tile kernel（及其符号 VJP 反向）；
2. `tg.mean()` → 已证明逐分量可加的元组状态：两个段归约求和加
   最后一次除法，带有生成的 VJP；
3. `tg.prod()` → 已证明的乘积幺半群，提升为零安全的乘积 IR，带有
   生成的 VJP；
4. `tg.online_softmax()` → tile 化的稳定流式路径；nn 子图分数进一步
...
`reducer.mlir(message_dtypes=(tg.float32,), symbol="user_mean")` 会把
