...
## 带 cu_seqlens 的 varlen 因果注意力 { #varlen-causal-attention-with-cu_seqlens }
...
该示例用一行 `gf.Graph.cu_seqlens(cu_seqlens, causal=True)` 构建块对角因果
CSR，并运行同一个无掩码边 UDF——包括一个长度为 1 的序列。同一个关系也可以写成
逐序列块的组合 `gf.Graph.cat([gf.Graph.triangular(L) for L in lengths])`：
块 `k` 只在内部互相注意，两种构造器产出完全相同的 CSR。（测试会将全部三个注意力示例与
...
### 稀疏与剪枝 { #sparse-and-pruned }

## 基于 tile 剪枝的稀疏注意力 { #tile-pruned-sparse-attention }

**它是什么。** 与第一个示例相同的注意力，但是近似的：tile 剪枝把 key 按块
（tile）处理，跳过任何最高分数已远低于当前行最大值的 tile——softmax 之后
其总权重可以忽略，因此输出在有界误差下便宜得多。FlashAttention 正是
每个 tile 都被接纳的精确特例；本示例添加了接纳规则。

![8×8 attention 矩阵按 2×2 tile 分组：靠近对角线的 tile 被接纳（实心靛蓝），其余被剪枝（打叉）——被剪枝 tile 的分数和 value 永远不会被读取。](../assets/examples/tile-pruned-attention.svg)

$$s_{hij} = \langle k_{hj},\, q_{hi}\rangle \cdot D^{-1/2}, \qquad
\mathrm{out}_{hi} = \sum_{j} \frac{e^{s_{hij}}}{\sum_{l} e^{s_{hil}}}\, v_{hj}$$

源 tile $T$ 仅当
$\max_{j \in T} s_{hij} \ge m^{\mathrm{run}}_{hi} - \tau$ 时才被接纳，其中
$m^{\mathrm{run}}_{hi}$ 是当前行最大值，$\tau$ 即
`block_prune_threshold`。

阈值的工作机制：这是内置 `gf.online_softmax()` reducer 的一个**构造参数，
...
subclass `gf.Reducer` 无法复刻这个机制：structured-dense lowering 只接受内置
...
UDF 沿用第一个示例的结构，只有一处变化——`gf.nn.trace(attn)` 产生分数。
