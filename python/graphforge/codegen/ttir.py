...
GraphForge's stable IR remains ``gf.kernel``.  A Triton provider plugin may
lower it to TTIR text and pass that text through this boundary.  Keeping the
input serialized avoids linking GraphForge's MLIR libraries and a vendor
...
def compile_ttir(
    ...
    *,
    ...
    worker_mode = os.environ.get("GRAPHFORGE_COMPILE_WORKER", "1").lower()
    if worker_mode not in {"0", "1", "false", "true", "off", "on"}:
        raise ValueError("GRAPHFORGE_COMPILE_WORKER must be 0/1/false/true/off/on")
    ...
    with TemporaryDirectory(prefix="graphforge-ttir-") as directory:
    ...
    launcher = os.environ.get(
        "GRAPHFORGE_CUDA_LAUNCHER", "triton"
