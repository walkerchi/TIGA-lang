...
class Graph:
    """PyTorch compatibility realization of a GraphForge logical relation.
    ...
    def __init__(
        ...
        *,
        ...
        self._cell_offsets_cache: torch.Tensor | None = None
    ...
    def halo(
        ...
        *,
        ...
        if not isinstance(mesh, DeviceMesh):
            raise TypeError("mesh must be a graphforge.DeviceMesh")
        if partition is None:
            partition = ByDestination()
        if not isinstance(partition, ByDestination):
            raise TypeError("partition must be a graphforge.ByDestination")
    ...
    def resolve_csr(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._schema.realization == "implicit_dense":
            ...
            if self._dense_boundary == "lower_inclusive":
                ...
                columns = [
                    torch.arange(
                        row + 1, device=self.device,
                        dtype=self._schema.index_dtype,
                    )
                    for row in range(self._schema.num_dst)
                ]
                col_idx = torch.cat(columns) if columns else torch.empty(
