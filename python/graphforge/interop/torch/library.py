"""Dynamically registered ``torch.library`` facade for GraphForge UDFs."""
...
def register_message_passing(
    ...
    **params: Any,
    ...
    """Register a GraphForge UDF specialization as a functional Torch op.
    ...
    The real implementation enters GraphForge's normal compiler path; the
    ...
    qualified = f"graphforge::{op_name}"
    found = _REGISTERED.get(qualified)
    if found is not None:
        return found

    library = torch.library.Library("graphforge", "FRAGMENT")
    arguments = ", ".join(
    ...
    operation = getattr(getattr(torch.ops.graphforge, op_name), "default")
