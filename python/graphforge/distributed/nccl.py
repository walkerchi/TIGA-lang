...
def _candidate_libraries(explicit: str | os.PathLike[str] | None):
    if explicit is not None:
        yield Path(explicit)
    configured = os.environ.get("GRAPHFORGE_NCCL_LIBRARY")
    if configured:
    ...
    def __init__(self, path: str | os.PathLike[str] | None = None) -> None:
        ...
        if library is None:
            ...
            raise ModuleNotFoundError(
                ...
                f"GRAPHFORGE_NCCL_LIBRARY ({detail})"
            )
    ...
    def __init__(
        ...
        *,
        ...
        if capability[0] >= 12 and self._api.version < 22800:
            raise RuntimeError(
                f"GraphForge requires NCCL >= 2.28 on {self.device} "
    ...
    def exchange_device(
        ...
        *,
        ...
        if self.world_size == 1:
            ...
            # place where GraphForge claims an NCCL point-to-point data path.
