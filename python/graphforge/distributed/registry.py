...
class TransportCapabilities:
    ...
    def __post_init__(self) -> None:
        if self.abi_version != 1:
            raise ValueError("unsupported GraphForge transport ABI")
...
def discover_transports() -> tuple[str, ...]:
    global _DISCOVERED
    if not _DISCOVERED:
        for entry in metadata.entry_points(group="graphforge.transport"):
...
def get_transport_provider(name: str) -> TransportProvider:
    ...
    try:
        ...
        raise LookupError(
            f"GraphForge transport provider {name!r} is unavailable; installed: "
