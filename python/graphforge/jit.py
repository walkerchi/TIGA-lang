...
the decorated function into :func:`graphforge.control.repeat` /
:func:`graphforge.control.while_loop` calls at capture time. It is a spelling
layer only: the same primitives, lowering, VJP and fail-closed contracts
apply, and MessagePassing UDF regions are never AST-transformed.
...
def jit(function=None, *, max_iterations: int | None = None):
    ...
    return wraps(function)(transformed)
