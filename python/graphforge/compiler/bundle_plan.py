...
def translate_task_bundle(
    ...
    *,
    ...
    if not isinstance(task_ir, str) or '"gf_task.' not in task_ir:
        raise ValueError("task_ir must contain a GraphForge task plan")
    tool = find_gf_translate(gf_translate)
    if tool is None:
        raise FileNotFoundError(
            "gf-translate was not found; install graphforge-compiler or set "
            "GRAPHFORGE_TRANSLATE"
        )
    with TemporaryDirectory(prefix="graphforge-task-") as directory:
