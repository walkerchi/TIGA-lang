"""Loader for the in-process GraphForge MLIR compiler extension."""
...
def _load_native():
    ...
    try:
        ...
        if not candidates:
            raise RuntimeError(
                "the native GraphForge compiler extension is unavailable; "
                "build/install graphforge-compiler before using JIT or IR APIs"
            ) from original
        ...
        if specification is None or specification.loader is None:
            raise RuntimeError("could not load the native GraphForge compiler")
...
def compile_cpu(output):
    """Compile a Tensor DAG in-process through GraphForge MLIR and LLVM.
