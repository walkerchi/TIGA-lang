...
class ProviderCapabilities:
    ...
    def __post_init__(self) -> None:
        if self.abi_version != 1:
            raise ValueError("unsupported Tiga provider ABI")
...
def discover_providers() -> tuple[str, ...]:
    global _DISCOVERED
    if not _DISCOVERED:
        for entry in metadata.entry_points(group="tiga.codegen"):
...
def get_provider(name: str) -> CodegenProvider:
    ...
    try:
        ...
        raise LookupError(
            f"Tiga codegen provider {name!r} is unavailable; installed: "
