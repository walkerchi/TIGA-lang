...
class Reducer(Kernel):
    ...
    declaration; Tiga never attempts to prove it from Python source.
    """
...
class MeanReducer(Reducer):
    """Neighbour mean as tuple state ``(sum, count)``.
...
class ProductReducer(Reducer):
    """Multiplicative monoid; promoted to the zero-safe product IR."""
...
def mean(*, deterministic: bool = False) -> MeanReducer:
    return MeanReducer(deterministic=deterministic)


def prod(*, deterministic: bool = False) -> ProductReducer:
    return ProductReducer(deterministic=deterministic)
