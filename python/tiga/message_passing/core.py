...
def _validate_native_fields(role, fields, entities, device) -> None:
    ...
    for name, value in fields.items():
        if not isinstance(value, Tensor):
            raise TypeError(f"{role}.{name} must be a tiga.Tensor")
...
def _resolve_node_fields(graph, src, dst, ndata):
    """Normalize the homogeneous-graph ``ndata=`` sugar to src/dst bindings.

    Edges are directed (j→i) and ``src``/``dst`` name the two endpoint roles.
    On a homogeneous relation the same node set plays both roles, so one
    ``ndata=`` mapping binds every field to both.
    """
    if ndata is not None:
        if src is not None or dst is not None:
            raise TypeError("ndata= cannot be combined with src=/dst=")
        if graph.schema.num_src != graph.schema.num_dst:
            raise ValueError(
                "ndata= requires a homogeneous relation (num_src == num_dst); "
                "declare src= and dst= separately for bipartite graphs")
        src = dst = dict(ndata)
    if src is None or dst is None:
        raise TypeError(
            "message passing requires ndata= or both src= and dst= mappings")
    return src, dst
...
def _native_csr_forward(
    ...
    *,
    ...
    """Differentiable native MessagePassing over one static CSR snapshot.

    Shared by the direct ``__call__`` path and the paged executor, which
    feeds page-local CSR tiles with page-local dst/edge views and — for
    disk-resident src fields — pre-gathered edge-domain ``src_at_edge`` rows.
    Returns ``(output, reducer_lowering)``.
    """
    if src_at_edge is None:
        src_at_edge = {
            name: value.gather(col_idx) for name, value in src.items()
        }
    src_namespace = SimpleNamespace(**src_at_edge)
    ...
    message = _call_region(
        kernel.edge, (src_namespace, dst_at_edge, edge_at_edge), params)
    if isinstance(kernel.reducer, SumReducer):
    ...
    elif isinstance(kernel.reducer, OnlineSoftmaxReducer):
        if kernel.reducer.block_prune_threshold is not None:
            raise NotImplementedError(
        ...
        aggregate = _apply_additive_reducer(
            kernel.reducer, message, row_ptr, graph.schema.num_dst)
        from ..compiler.reducer_capture import capture_reducer
    ...
    output = (
        aggregate if type(kernel).node is MessagePassing.node
        else _call_region(
...
class MessagePassing(Kernel):
    ...
    def prepare(
        ...
        *,
        graph: Graph,
        src: Mapping[str, Tensor | object] | None = None,
        dst: Mapping[str, Tensor | object] | None = None,
    ...
    def __call__(
        ...
        **params: Any,
        ...
        src, dst = _resolve_node_fields(graph, src, dst, ndata)
        edge = {} if edge is None else edge
        from ..control import staging_active
        from ..program import active_program

        capture = active_program()
        if capture is not None and not staging_active():
        ...
        if graph.schema.realization == "paged_csr":
            from .paged import execute_paged_message_passing

            page_rows = params.pop("page_rows", None)
            prefetch = params.pop("prefetch", True)
            prefetch_depth = params.pop("prefetch_depth", None)
            return execute_paged_message_passing(
                self,
                graph=graph,
                src=src,
                dst=dst,
        ...
        output, reducer_lowering = _native_csr_forward(
            self,
            graph=graph,
            row_ptr=row_ptr,
            col_idx=col_idx,
