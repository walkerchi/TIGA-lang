"""Torch-independent Tiga execution runtime.
...
class Device:
    ...
    def parse(cls, value: Device | str) -> Device:
        ...
        if not isinstance(value, str):
            raise TypeError("device must be a tiga.runtime.Device or string")
...
def _library_candidates() -> list[Path]:
    override = os.environ.get("TIGA_RUNTIME_LIBRARY")
...
class Buffer:
    ...
    def wrap_address(
        ...
        *,
        ...
        released. The C runtime receives no deleter, so Tiga can use the
...
class RuntimeKernel:
    ...
    def prepare_async(
        ...
        *,
        ...
        deliberately avoids allocating a redundant Tiga event.
        """
        self._require_open()
        resolved_grid = _launch_shape("grid", grid)
        resolved_block = _launch_shape("block", block)
...
def __getattr__(name: str):  # lazy: graph ↔ runtime import cycle
    if name == "auto_offload":
        from ..graph.offload import auto_offload
