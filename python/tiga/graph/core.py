...
realizers needed by native Tiga tensors. It contains no topology builder
...
or inspection. Optional framework inputs are dispatched to ``tiga.interop``.
"""
...
def _block_csr_lists(block: "Graph") -> tuple[list[int], list[int]]:
    """Resolve one ``cat`` block to Python-list CSR on CPU without Torch."""
    if block._row_ptr is not None:
        return block._row_ptr.tolist(), block._col_idx.tolist()
    if block._schema.realization == "implicit_dense":
        num_src, num_dst = block._schema.num_src, block._schema.num_dst
        if block._dense_boundary == "lower_inclusive":
            rows = [row * (row + 1) // 2 for row in range(num_dst + 1)]
            columns = [
                column for row in range(num_dst) for column in range(row + 1)]
        else:
            rows = [row * num_src for row in range(num_dst + 1)]
            columns = list(range(num_src)) * num_dst
        return rows, columns
    row_ptr, col_idx = block.resolve_csr()
    return row_ptr.tolist(), col_idx.tolist()


class Graph:
    """Immutable logical relation over Tiga tensors.
    ...
    def _validate_index_tensor(name: str, value: Tensor) -> None:
        if not isinstance(value, Tensor):
            raise TypeError(f"{name} must be a tiga.Tensor")
        if value.ndim != 1:
    ...
    @classmethod
    def regular(cls, num_nodes: int, degree: int, *, device: Device | str = "cpu"):
        ...
        if parsed.type is not DeviceType.CPU:
            from ..interop.torch.graph import Graph as TorchGraph
            from ..tensor import from_torch

            adapted = TorchGraph.regular(num_nodes, degree, device=str(parsed))
            row_ptr, col_idx = adapted.resolve_csr()
            return cls.from_csr(
                from_torch(row_ptr), from_torch(col_idx), num_src=num_nodes)
        rows = [index * degree for index in range(num_nodes + 1)]
        columns = [edge % num_nodes for edge in range(num_nodes * degree)]
        return cls.from_csr(
            tensor(rows, dtype=int64, device=parsed),
            tensor(columns, dtype=int64, device=parsed),
            num_src=num_nodes,
        )

    @classmethod
    def cu_seqlens(cls, cu_seqlens, *, causal: bool = True):
        ...
        sequence ``[s_k, s_{k+1})`` (``causal=False``).  Torch inputs build
        vectorized on their own device through the interop mirror.
        """
        if not cls._is_native(cu_seqlens):
            from ..interop.torch.graph import Graph as TorchGraph
            from ..tensor import from_torch

            adapted = TorchGraph.cu_seqlens(cu_seqlens, causal=causal)
            row_ptr, col_idx = adapted.resolve_csr()
            return cls.from_csr(
                from_torch(row_ptr), from_torch(col_idx),
                num_src=adapted.schema.num_src)
        ...
        return cls.from_csr(
            tensor(rows, dtype=cu_seqlens.dtype, device=cu_seqlens.device),
            tensor(columns, dtype=cu_seqlens.dtype, device=cu_seqlens.device),
            num_src=boundaries[-1],
        )
    ...
    def stencil(
        ...
        *,
        ...
        if parsed.type is not DeviceType.CPU:
            ...
            adapted = TorchGraph.stencil(
                dims, offsets, periodic=periodic, device=str(parsed))
            row_ptr, col_idx = adapted.resolve_csr()
            return cls.from_csr(
                from_torch(row_ptr), from_torch(col_idx),
                num_src=adapted.schema.num_src)
        num_nodes = math.prod(dims)
        strides = tuple(math.prod(dims[axis + 1:]) for axis in range(len(dims)))
        rows = [0]
        columns: list[int] = []
        for destination in range(num_nodes):
            ...
            for stride in strides:
                ...
                remainder %= stride
            for offset in offsets:
                source = [axis + shift for axis, shift in zip(coordinate, offset)]
                if periodic:
                    source = [
                        value % extent for value, extent in zip(source, dims)]
                elif any(
                        value < 0 or value >= extent
                        for value, extent in zip(source, dims)):
                    continue
                columns.append(sum(
                    value * stride for value, stride in zip(source, strides)))
            rows.append(len(columns))
        return cls.from_csr(
            tensor(rows, dtype=int64, device=parsed),
            tensor(columns, dtype=int64, device=parsed),
        ...
        *,
        ...
        if periodic is not None:
            if not isinstance(periodic, Tensor):
                raise TypeError("periodic must be a tiga.Tensor")
        ...
        for name, value in builder_fields.items():
            if not isinstance(value, Tensor):
                raise TypeError(f"fields[{name!r}] must be a tiga.Tensor")
    ...
    def dense(
        ...
        *,
        ...
        # Tiga Devices remain native and do not import Torch.
    ...
    def cat(cls, graphs):
        ...
        ``cat([Graph.triangular(n) for n in lengths])`` equals
        ``Graph.cu_seqlens(cumsum(lengths), causal=True)`` and
        ``cat([Graph.dense(n) for n in lengths])`` equals the ``causal=False``
        variant.  The composition realizes one materialized CSR snapshot;
        non-CPU inputs build vectorized on their own device through the
        interop mirror.
        """
        if not isinstance(graphs, (list, tuple)):
            raise TypeError("graphs must be a list or tuple of Graph")
        blocks = list(graphs)
        if not blocks:
        ...
        for block in blocks:
            ...
            if block.is_distributed:
                raise ValueError("cannot cat a distributed graph")
        device = blocks[0].device
        if any(
            (block.device.type, block.device.ordinal)
            != (device.type, device.ordinal)
            for block in blocks
        ):
            raise ValueError("all blocks must share a device")
        index_dtype = (
            int64
            if any(block.schema.index_dtype is int64 for block in blocks)
            else int32
        )
        if device.type is not DeviceType.CPU:
            from ..interop.torch.graph import Graph as TorchGraph, from_native
            from ..tensor import from_torch

            adapted = TorchGraph.cat([from_native(block) for block in blocks])
            row_ptr, col_idx = adapted.resolve_csr()
            return cls.from_csr(
                from_torch(row_ptr), from_torch(col_idx),
                num_src=adapted.schema.num_src)
        ...
        src_offset = 0
        for block in blocks:
            block_rows, block_columns = _block_csr_lists(block)
            base = rows[-1]
            rows.extend(value + base for value in block_rows[1:])
            columns.extend(column + src_offset for column in block_columns)
    ...
    def paged_page(self, begin: int, end: int):
        """One bounded CSR destination tile plus its global edge offset."""
        if self._paged_store is None:
            raise TypeError("Graph is not backed by a paged CSR PhysicalInstance")
        return self._paged_store.read_page(begin, end)
    ...
    def fields(self, role: str, *, requires_grad: bool | None = None) -> dict[str, Tensor]:
        ...
        shells: dict[str, Tensor] = {}
        for name in self._paged_store.field_names(role):
            shape, dtype = self._paged_store.field_shape_dtype(role, name)
            shell = Tensor(
                shape,
                dtype=dtype,
                device="cpu",
