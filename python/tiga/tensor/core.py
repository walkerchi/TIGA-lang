"""Minimal Torch-independent Tensor frontend for Tiga.
...
class Tensor:
    """A logical strided value backed by a Tiga runtime allocation."""
    ...
    def __init__(
        ...
        *,
        ...
        if dtype.name not in _DTYPES or _DTYPES[dtype.name] is not dtype:
            raise TypeError("dtype must be a Tiga DType")
        self.dtype = dtype
        ...
        self._buffer = buffer
        self._spill: dict[str, object] | None = None
    ...
    def __sub__(self, other: Tensor | int | float | complex) -> Tensor:
        from ..program import ProgramValue, leaf_tensor

        if isinstance(other, ProgramValue):
            other = leaf_tensor(other)
    ...
    def matmul(self, other: Tensor) -> Tensor:
        ...
        if not isinstance(other, Tensor):
            raise TypeError("Tensor.matmul requires another tiga.Tensor")
    ...
    def gather(self, index: Tensor) -> Tensor:
        ...
        if not isinstance(index, Tensor) or index.ndim != 1:
            raise TypeError("gather index must be a rank-one tiga.Tensor")
    ...
    def segment_sum(self, index: Tensor, num_segments: int) -> Tensor:
        ...
        if not isinstance(index, Tensor) or index.ndim != 1:
            raise TypeError("segment index must be a rank-one tiga.Tensor")
        if index.dtype.kind != "int":
    ...
    def realize(self) -> Tensor:
        ...
        if paged_field is not None:
            # Disk-resident .gfg field shell: an explicit materialization
            # request reads the whole payload back into RAM.  Paged execution
            # never lands here — it consumes bounded row ranges directly.
            values = paged_field.read_rows(0, self.shape[0])
            self._buffer = Buffer(self.nbytes, device=self.device, _pooled=True)
            self.strides = _contiguous_strides(self.shape)
            self.offset = 0
            self._write_flat(values)
            self.ready_event = Event(self.device, _pooled=True)
            self._execution_info = {"backend": "paged-field-reload"}
            return self
        requested_backend = os.environ.get(
            "TIGA_TENSOR_BACKEND", "auto"
        ).lower()
        if requested_backend not in {"auto", "native", "python"}:
            raise ValueError(
                "TIGA_TENSOR_BACKEND must be auto, native or python"
            )
        largest_value = max(
        ...
        if self._expr is not None and self._expr.op == "program_value":
            # A program leaf has no Tensor IR of its own: realizing it means
            # running the owning GraphProgram, not compiling the boundary.
            use_native = False
        if use_native:
            try:
    ...
    def _evaluate_flat(self, memo: dict[int, list[object]] | None = None) -> list[object]:
        ...
        if self._expr is None:
            paged_field = getattr(self, "_paged_field", None)
            if paged_field is not None:
                # A .gfg field shell evaluated outside paged execution reads
                # its whole payload; paged pages never take this branch.
                result = paged_field.read_rows(0, self.shape[0])
                cache[key] = result
                return result
        ...
        if expression.op in {"repeat", "while"}:
        ...
        elif expression.op == "program_value":
            ...
            if producer not in observed:
                raise RuntimeError(
                    "program leaf was not among the observed GraphProgram outputs")
            value = observed[producer]
            if isinstance(value, Tensor):
                result = (value._read_flat() if value._buffer is not None
                          else value._evaluate_flat(cache))
            else:
    ...
    def to_numpy(self):
        ...
        NumPy remains optional: importing Tiga does not import or depend
    ...
    def to_torch(self, *, copy: bool = False):
        ...
        H2H/D2D copy from Tiga-owned contiguous storage.
        """
        if copy:
    ...
    def expression(self) -> str:
        """Return a debug capture graph; this is not canonical Tiga MLIR."""
        lines: list[str] = []
        names: dict[int, str] = {}
...
def _program_leaf_input(node: Tensor, *, realize: bool) -> Tensor:
    """Materialize one ``program_value`` node as a plain input Tensor.

    With ``realize=False`` this is inspection-only: an unrealized input
    placeholder, no program execution. With ``realize=True`` the owning
    GraphProgram runs once per call (fusion plan included) and the leaf
    result's storage backs the substitute zero-copy.
    """
    if not realize:
        return Tensor(
            node.shape, dtype=node.dtype, device=node.device,
    ...
    program = expression.attr("program")
    producer = int(expression.attr("producer"))
    observed = program._observed_results()
    if producer not in observed:
        raise RuntimeError(
            "program leaf was not among the observed GraphProgram outputs")
    result = observed[producer]
    if not isinstance(result, Tensor):
        result = from_torch(result)
    material = result if result._buffer is not None else result.realize()
    return Tensor(
        node.shape, dtype=node.dtype, device=node.device,
        buffer=material._buffer, offset=material.offset,
        strides=material.strides, version=node.version,
        ready_event=material.ready_event,
    )


def _substitute_program_leaves(root: Tensor, *, realize: bool) -> Tensor:
    """Clone an expression DAG with program leaves replaced by plain inputs.

    A ``program_value`` node is a stage boundary owned by its GraphProgram,
    not a Tensor IR operation; native capture and compilation consume the
    leaf result as an ordinary input instead. Control regions are rebuilt
    consistently (arguments, body, condition and sibling results), keeping
    the explicit capture ABI intact. The user-visible DAG is never mutated —
    only the returned clone is cut — and without any program leaves the
    original root is returned unchanged.
    """

    def scan(value: Tensor, seen: set[int]) -> bool:
        if id(value) in seen:
            return False
        seen.add(id(value))
        ...
        if expression.op == "program_value":
            return True
        if any(scan(operand, seen) for operand in expression.operands):
            return True
        region = expression.region
        if region is None:
            return False
        ...
        if any(scan(output, seen) for output in outputs):
            return True
        return region.condition is not None and scan(region.condition, seen)

    if not scan(root, set()):
        return root

    memo: dict[int, Tensor] = {}
    regions: dict[int, _Region] = {}

    def rebuild_region(region: _Region, operands: tuple[Tensor, ...]) -> _Region:
        arguments = tuple(visit(argument) for argument in region.arguments)
        outputs = (region.output,) if isinstance(region.output, Tensor) \
            else region.output
        new_outputs = tuple(visit(output) for output in outputs)
        condition = None if region.condition is None else visit(region.condition)
        rebuilt = _Region(
            arguments,
            new_outputs[0] if isinstance(region.output, Tensor) else new_outputs,
            condition=condition,
        )
        # Control results are siblings over one region; rebuild them together
        # so the native builder can type the multi-result op from any root.
        for old_result in region.results:
            sibling = old_result._expr
            clone = Tensor(
                old_result.shape, dtype=old_result.dtype,
                device=old_result.device,
                requires_grad=old_result.requires_grad,
                expression=_Expr(
                    sibling.op, operands, sibling.attrs, rebuilt),
                version=old_result.version,
            )
            memo[id(old_result)] = clone
        rebuilt.results = tuple(
            memo[id(old_result)] for old_result in region.results)
        return rebuilt
    ...
    def visit(value: Tensor) -> Tensor:
        ...
        if found is not None:
            return found
        expression = value._expr
        if expression is None or expression.op == "loop_argument":
            memo[id(value)] = value
            return value
        if expression.op == "program_value":
            clone = _program_leaf_input(value, realize=realize)
            memo[id(value)] = clone
            return clone
        if value._buffer is not None:
            # Already-realized nodes are cut at the executable ABI by the
            # ordinary physicalization pass; keep the original object.
            memo[id(value)] = value
            return value
        operands = tuple(visit(operand) for operand in expression.operands)
        if expression.region is not None and id(expression.region) not in regions:
            regions[id(expression.region)] = rebuild_region(
                expression.region, operands)
            # rebuild_region registered clones for all sibling results.
            return memo[id(value)]
        region = regions.get(id(expression.region))
        found = memo.get(id(value))
        if found is not None:
            return found
        clone = Tensor(
            ...
            expression=_Expr(expression.op, operands, expression.attrs, region),
