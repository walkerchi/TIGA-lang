...
_EDGE_NN_TILE_PLANS: dict[object, object] = {}


def _edge_nn_tile_plan(spec):
    """Compile-and-cache one emitted edge nn tile kernel per structure."""
    from ...codegen import prepare_ttir_edge_nn_tile
    from ...compiler.edge_nn_tile import emit_edge_nn_tile_ttir

    plan = _EDGE_NN_TILE_PLANS.get(spec)
    if plan is None:
        if len(_EDGE_NN_TILE_PLANS) > 64:
            _EDGE_NN_TILE_PLANS.clear()
        plan = prepare_ttir_edge_nn_tile(
            emit_edge_nn_tile_ttir(spec),
            out_width=spec.out_width,
            block_e=spec.block_e,
            num_warps=spec.num_warps,
        )
        _EDGE_NN_TILE_PLANS[spec] = plan
    return plan


class _EdgeNNWeightCache:
    """Per-parameter padded weight cache safe against allocator reuse.

    Entries are keyed by ``id(parameter)`` but validated by a weak
    reference plus the autograd version counter: a dead tensor's id may be
    recycled, yet the identity check can never alias a different live
    tensor.  (``WeakKeyDictionary`` itself is unusable here because its
    collision probe calls ``Tensor.__eq__``.)
    """

    def __init__(self) -> None:
        self._entries: dict[int, tuple[object, object, object]] = {}

    def weight(self, parameter, in_pad: int, out_pad: int):
        return self._lookup(
            parameter, (in_pad, out_pad),
            lambda: self._pad(parameter, in_pad, out_pad, transpose=True))

    def bias(self, parameter, out_pad: int):
        return self._lookup(
            parameter, (out_pad,),
            lambda: self._pad(parameter, out_pad, out_pad, transpose=False))

    def _lookup(self, parameter, shape_key, build):
        import weakref
    ...
    @staticmethod
    def _pad(parameter, rows: int, cols: int, *, transpose: bool):
        if transpose:
            padded = torch.zeros(
                (rows, cols), dtype=parameter.dtype, device=parameter.device)
            padded[: parameter.shape[1], : parameter.shape[0]] = (
                parameter.detach().t())
        else:
            padded = torch.zeros(
                rows, dtype=parameter.dtype, device=parameter.device)
            padded[: parameter.shape[0]] = parameter.detach()
        return padded


_EDGE_NN_WEIGHT_CACHE = _EdgeNNWeightCache()


def _edge_nn_param_slots(spec):
    """Kernel parameter slots of one edge nn spec, in launch order.

    Each slot is ``(name, kind, padded_shape, true_shape)`` where ``kind``
    is ``"matrix"`` (a Linear weight padded to ``[in_pad, out_pad]``) or
    ``"vector"`` (a Linear bias or LayerNorm γ/β padded to ``[width_pad]``).
    Linear weights are stored transposed in the kernel; the true shape is
    what the host slices back out of the gradient buffers.
    """
    from ...compiler.edge_nn_tile import _pad16
    from ...compiler.nn_capture import NormLayer
    ...
    for layer in spec.layers:
        ...
        if isinstance(layer, NormLayer):
            width_pad = _pad16(layer.width)
            slots.append((
                layer.weight_name, "vector", (width_pad,), (layer.width,)))
            if layer.bias_name is not None:
                slots.append((
                    layer.bias_name, "vector", (width_pad,), (layer.width,)))
            continue
        out_pad = _pad16(layer.out_features)
        slots.append((
            layer.weight_name, "matrix", (in_pad, out_pad),
            (layer.in_features, layer.out_features)))
        if layer.bias_name is not None:
            slots.append((
                layer.bias_name, "vector", (out_pad,), (layer.out_features,)))
        in_pad = out_pad
    return slots


_EDGE_NN_VJP_PLANS: dict[object, object] = {}


_EDGE_NN_ATTENTION_PLANS: dict[object, object] = {}


def _edge_nn_attention_plan(spec):
    """Compile-and-cache one emitted nn-attention kernel per structure."""
    from ...codegen import prepare_ttir_edge_nn_attention
    from ...compiler.edge_nn_attention import emit_edge_nn_attention_ttir

    plan = _EDGE_NN_ATTENTION_PLANS.get(spec)
    if plan is None:
        if len(_EDGE_NN_ATTENTION_PLANS) > 64:
            _EDGE_NN_ATTENTION_PLANS.clear()
        plan = prepare_ttir_edge_nn_attention(
            emit_edge_nn_attention_ttir(spec),
            out_width=spec.value_width,
            num_warps=spec.score.num_warps,
        )
        _EDGE_NN_ATTENTION_PLANS[spec] = plan
    return plan


_EDGE_NN_ATTENTION_VJP_PLANS: dict[object, object] = {}


def _edge_nn_attention_vjp_plan(spec):
    """Compile-and-cache one emitted nn-attention backward kernel."""
    from ...codegen import prepare_ttir_edge_nn_attention_vjp
    from ...compiler.edge_nn_attention import emit_edge_nn_attention_vjp_ttir

    plan = _EDGE_NN_ATTENTION_VJP_PLANS.get(spec)
    if plan is None:
        if len(_EDGE_NN_ATTENTION_VJP_PLANS) > 64:
            _EDGE_NN_ATTENTION_VJP_PLANS.clear()
        plan = prepare_ttir_edge_nn_attention_vjp(
            emit_edge_nn_attention_vjp_ttir(spec),
            num_warps=spec.score.num_warps,
        )
        _EDGE_NN_ATTENTION_VJP_PLANS[spec] = plan
    return plan


class _EdgeNNAttentionFunction(torch.autograd.Function):
    """Autograd bridge: fused attention forward + fused row-centric backward.

    Forward persists only the O(N) softmax state (m, l) plus the inputs;
    backward replays the score chain per row chunk and applies the softmax
    Jacobian adjoint in-tile, so no [E, ·] tensor exists in either direction.
    """

    @staticmethod
    def forward(ctx, fwd_plan, vjp_plan, spec, row_i32, col_i32, num_rows,
                *tensors):
        score_spec = spec.score
        n_seg = sum(1 for s in score_spec.segments if s.kind != "displacement")
        segments = tensors[:n_seg]
        positions = tensors[n_seg] if score_spec.needs_positions else None
        rest = tensors[n_seg + bool(score_spec.needs_positions):]
        parameters, value = rest[:-1], rest[-1]
        padded = []
        for parameter, (_name, kind, shape, _true) in zip(
                parameters, _edge_nn_param_slots(score_spec)):
            if kind == "matrix":
                padded.append(_EDGE_NN_WEIGHT_CACHE.weight(
                    parameter, *shape))
            else:
                padded.append(_EDGE_NN_WEIGHT_CACHE.bias(
                    parameter, shape[0]))
        launch = [row_i32, col_i32, *segments]
        if positions is not None:
            launch.append(positions)
        launch.extend(padded)
        launch.append(value)
        output, m, l = fwd_plan.run(tuple(launch), num_rows=num_rows)
        ctx.save_for_backward(
            row_i32, col_i32, *segments,
            *([positions] if positions is not None else []), *padded,
            value, output, m, l)
        ctx.vjp_plan = vjp_plan
        ctx.spec = spec
    ...
    def backward(ctx, d_out):
        ...
        row_i32, col_i32 = saved[0], saved[1]
        segments = saved[2:2 + n_seg]
        positions = saved[2 + n_seg] if score_spec.needs_positions else None
        rest = saved[2 + n_seg + bool(score_spec.needs_positions):]
        n_params = len(_edge_nn_param_slots(score_spec))
        padded = rest[:n_params]
        value, output, m, l = rest[n_params:]
        d_out = d_out.contiguous()
        d_segments = [torch.zeros_like(segment) for segment in segments]
        d_positions = (
            torch.zeros_like(positions) if positions is not None else None)
        grad_buffers = [
            torch.zeros(shape, dtype=d_out.dtype, device=d_out.device)
            for _name, _kind, shape, _true in _edge_nn_param_slots(score_spec)
        ]
        ...
        if positions is not None:
            launch.append(positions)
        launch.extend(padded)
        launch.extend((value, output, m, l, d_out))
        launch.extend(d_segments)
        if d_positions is not None:
            launch.append(d_positions)
        launch.extend(grad_buffers)
        launch.append(d_value)
        ctx.vjp_plan.run(tuple(launch), num_rows=ctx.num_rows)
        grads = list(d_segments)
        if score_spec.needs_positions:
            grads.append(d_positions)
        for buffer, (_name, kind, _shape, true) in zip(
                grad_buffers, _edge_nn_param_slots(score_spec)):
            if kind == "matrix":
                grads.append(buffer[: true[0], : true[1]].t().contiguous())
            else:
                grads.append(buffer[: true[0]].contiguous())
        grads.append(d_value)
        return (None, None, None, None, None, None, *grads)


def _edge_nn_vjp_plan(spec):
    """Compile-and-cache one emitted edge nn backward kernel per structure."""
    from ...codegen import prepare_ttir_edge_nn_vjp
    from ...compiler.edge_nn_vjp import emit_edge_nn_vjp_ttir

    plan = _EDGE_NN_VJP_PLANS.get(spec)
    if plan is None:
        if len(_EDGE_NN_VJP_PLANS) > 64:
            _EDGE_NN_VJP_PLANS.clear()
        plan = prepare_ttir_edge_nn_vjp(
            emit_edge_nn_vjp_ttir(spec),
            block_e=spec.block_e,
            num_warps=spec.num_warps,
        )
        _EDGE_NN_VJP_PLANS[spec] = plan
    return plan


class _EdgeNNTileVjpFunction(torch.autograd.Function):
    """Autograd bridge: fused tile forward + fused recompute-tile backward.

    Forward saves only the O(N) inputs (field tensors, positions, padded
    weights); backward replays the chain tile-locally with the symbolic VJP
    rules, so training an edge nn materializes no [E, ·] tensor in either
    direction.
    """

    @staticmethod
    def forward(ctx, fwd_plan, vjp_plan, spec, dst_i32, src_i32, num_edges,
                num_rows, *tensors):
        n_seg = sum(1 for s in spec.segments if s.kind != "displacement")
        segments = tensors[:n_seg]
        positions = tensors[n_seg] if spec.needs_positions else None
        parameters = tensors[n_seg + bool(spec.needs_positions):]
        padded = []
        for parameter, (_name, kind, shape, _true) in zip(
                parameters, _edge_nn_param_slots(spec)):
            if kind == "matrix":
                padded.append(_EDGE_NN_WEIGHT_CACHE.weight(
                    parameter, *shape))
            else:
                padded.append(_EDGE_NN_WEIGHT_CACHE.bias(
                    parameter, shape[0]))
        launch = [dst_i32, src_i32, *segments]
        if positions is not None:
            launch.append(positions)
        launch.extend(padded)
        output = fwd_plan.run(
            tuple(launch), num_edges=num_edges, num_rows=num_rows)
        ctx.save_for_backward(
            dst_i32, src_i32, *segments,
            *([positions] if positions is not None else []), *padded)
        ctx.vjp_plan = vjp_plan
        ctx.spec = spec
        ctx.num_edges = num_edges
        # The plan reuses its output buffer across launches; the graph owns
        # a copy.
        return output.clone()
    ...
    def backward(ctx, d_out):
        ...
        n_seg = sum(1 for s in spec.segments if s.kind != "displacement")
        saved = ctx.saved_tensors
        dst_i32, src_i32 = saved[0], saved[1]
        segments = saved[2:2 + n_seg]
        positions = saved[2 + n_seg] if spec.needs_positions else None
        padded = saved[2 + n_seg + bool(spec.needs_positions):]
        d_out = d_out.contiguous()
        d_segments = [torch.zeros_like(segment) for segment in segments]
        d_positions = (
            torch.zeros_like(positions) if positions is not None else None)
        grad_buffers = [
            torch.zeros(shape, dtype=d_out.dtype, device=d_out.device)
            for _name, _kind, shape, _true in _edge_nn_param_slots(spec)
        ]
        launch = [dst_i32, src_i32, *segments]
        if positions is not None:
            launch.append(positions)
        ...
        if d_positions is not None:
            launch.append(d_positions)
        launch.extend(grad_buffers)
        ctx.vjp_plan.run(tuple(launch), num_edges=ctx.num_edges)
        grads = list(d_segments)
        if spec.needs_positions:
            grads.append(d_positions)
        for buffer, (_name, kind, _shape, true) in zip(
                grad_buffers, _edge_nn_param_slots(spec)):
            if kind == "matrix":
                grads.append(buffer[: true[0], : true[1]].t().contiguous())
            else:
                grads.append(buffer[: true[0]].contiguous())
...
class _GeneratedRadiusExecutable:
    ...
    def _materialize_if_profitable(self, positions: torch.Tensor) -> None:
        ...
        budget = int(os.environ.get(
            "TIGA_RADIUS_CACHE_BYTES", str(512 << 20)))
...
@dataclass
class _EdgeNNTileExecutable:
    """Guarded frozen launch for one compiled edge-NN tile kernel.

    Guards: graph identity, CSR snapshot pointer+version, per-field
    dtype/shape/device/stride, weight parameter identity+version, positions
    pointer+version, and the no-autograd-tape contract.  A miss falls back
    to the full dispatch, which recompiles or routes to the eager oracle.
    """

    graph: Graph
    csr_guard: tuple[object, ...]
    field_specs: dict[str, dict[str, tuple[object, ...]]]
    weight_guards: tuple[tuple[object, int], ...]
    positions_guard: tuple[object, int] | None
    spec: object
    plan: object
    dst_i32: torch.Tensor
    src_i32: torch.Tensor
    ...
    def try_run(self, graph, src, dst, edge, params=None):
        ...
        if graph is not self.graph:
            return _EXECUTABLE_MISS
        tensors = [
            value for mapping in (src, dst, edge) for value in mapping.values()]
        ...
        for role, mapping in (("src", src), ("dst", dst), ("edge", edge)):
            specs = self.field_specs[role]
            if set(mapping) != set(specs):
                return _EXECUTABLE_MISS
        ...
        for segment in spec.segments:
            if segment.kind == "displacement":
                continue
            mapping = {
                "src_field": src, "dst_field": dst, "edge_field": edge,
            }[segment.kind]
            value = mapping[segment.name]
            segment_tensors.append(
                value if value.is_contiguous() else value.contiguous())
        launch_args = [self.dst_i32, self.src_i32, *segment_tensors]
        if spec.needs_positions:
            launch_args.append(positions)
        launch_args.extend(self.padded)
...
class MessagePassing(Kernel):
    ...
    def __call__(
        ...
        **params: Any,
        ...
        if pattern is not None:
            ...
            if result is not None:
                return result
        result = self._execute_edge_nn_tile(
    ...
    def _execute_edge_nn_tile(
        self, key, graph, row_ptr, col_idx, src, dst, edge, params
    ):
        """Compile one captured edge nn subgraph to a fused tile kernel.

        The Python emission backend (phase 1) handles forward-only,
        float32, sum-reduced messages whose nn chain inputs are per-edge
        field gathers.  Anything outside that envelope returns ``None`` and
        the eager oracle below owns the semantics.
        """
        if graph.device.type != "cuda":
            return None
        if type(self).node is not MessagePassing.node:
            return None
        from ...compiler.capture import capture_edge_staged

        tensors = [
            value for mapping in (src, dst, edge) for value in mapping.values()]
        captured = capture_edge_staged(self.edge, params)
        if captured is None:
            return None
        from ...compiler.capture import Expr
        if not isinstance(captured, Expr):
            # Staged reducer item: an online-softmax score/value pair whose
            # score is an nn subgraph takes the row-centric attention tile.
            from ...reducer import OnlineSoftmaxItem
            if (
                isinstance(self.reducer, OnlineSoftmaxReducer)
                and isinstance(captured, OnlineSoftmaxItem)
            ):
                return self._execute_edge_nn_attention(
                    key, graph, row_ptr, col_idx, src, dst, edge, params,
                    captured, tensors)
            return None
        if not isinstance(self.reducer, SumReducer):
            return None
        from ...compiler.edge_nn_tile import (
            build_edge_nn_spec,
            emit_edge_nn_tile_ttir,
        )

        if captured.op != "nn_subgraph":
            return None
        dag = captured.args[0]
        weights = dict(dag.graph_module.named_parameters())
        if (
            any(value.dtype != torch.float32 for value in tensors)
        ...
        for role, mapping in (("src", src), ("dst", dst), ("edge", edge)):
            for name, value in mapping.items():
                if value.ndim not in (1, 2):
                    return None
                field_widths[(role, name)] = (
                    int(value.shape[1]) if value.ndim == 2 else 1)
        try:
            positions = graph.euclidean_positions()
        ...
        position_dim = (
            ...
            if positions is not None and positions.ndim == 2 else None)
        spec = build_edge_nn_spec(
            captured, field_widths=field_widths, position_dim=position_dim)
        if spec is None:
            return None
        ...
        for segment in spec.segments:
            ...
            segment_tensors.append(
                value if value.is_contiguous() else value.contiguous())
        if spec.needs_positions and not positions.is_contiguous():
            positions = positions.contiguous()

        padded = []
        for name, kind, shape, _true in _edge_nn_param_slots(spec):
            if kind == "matrix":
                padded.append(_EDGE_NN_WEIGHT_CACHE.weight(
                    weights[name], *shape))
            else:
                padded.append(_EDGE_NN_WEIGHT_CACHE.bias(
                    weights[name], shape[0]))
        ...
        requires_tape = torch.is_grad_enabled() and (
            any(value.requires_grad for value in tensors)
            or any(parameter.requires_grad for parameter in weights.values())
            or (spec.needs_positions and positions.requires_grad))
        if requires_tape:
            # Training path: the fused forward carries a fused recompute VJP,
            # so gradients never materialize an [E, ·] tensor either.
            return self._run_edge_nn_tile_vjp(
                key, graph, spec, dst_i32, src_i32, num_edges,
                segment_tensors, positions, weights)

        plan = _edge_nn_tile_plan(spec)
        launch_args = [dst_i32, src_i32, *segment_tensors]
        if spec.needs_positions:
            launch_args.append(positions)
        launch_args.extend(padded)
        output = plan.run(
            tuple(launch_args),
            num_edges=num_edges,
            num_rows=graph.schema.num_dst,
        ...
        variant = CompiledVariant(
            ...
            provider_key=plan.result.provider.cache_key(),
            domain_plan="\n".join((
                "// Tiga captured domain-plan",
                f"apply @{type(self).__qualname__}",
                "  recognize edge_nn_subgraph reducer(sum)",
                f"  segments {[f'{s.kind}:{s.name}x{s.width}' for s in spec.segments]}",
                f"  layers {[type(layer).__name__ for layer in spec.layers]}",
                (f"  edge_tile block_e={spec.block_e} k_pad={spec.k_pad} "
                 f"out={spec.out_width}/{spec.out_pad}"),
            )),
            passes=(
                "capture-edge-nn-subgraph",
                "translate-fx-to-message-dag",
                "prove-edge-nn-tile-structure",
                "python-emit-edge-nn-tile-ttir",
                "provider-compile-serialized-ttir",
            ),
            artifacts={"ttir": emit_edge_nn_tile_ttir(spec)},
            remarks=(
                ("Python emission backend (phase 1); the C++ "
                 "gf-kernel-to-ttir emitter replaces it in a later phase"),
                ("edge messages are evaluated inside the tile; no O(E) "
                 "message tensor is materialized"),
                ("inference path; calls that could request gradients take "
                 "the fused recompute VJP (gf-python-emit-edge-nn-tile-vjp)"),
            ),
        )
        self._record_variant(variant)
    ...
    def _run_edge_nn_tile_vjp(
        self, key, graph, spec, dst_i32, src_i32, num_edges,
        segment_tensors, positions, weights,
    ):
        """Training path: fused tile forward with a fused recompute VJP."""
        from ...compiler.edge_nn_tile import emit_edge_nn_tile_ttir
        from ...compiler.edge_nn_vjp import emit_edge_nn_vjp_ttir

        fwd_plan = _edge_nn_tile_plan(spec)
        vjp_plan = _edge_nn_vjp_plan(spec)
        parameters = [
            weights[name]
            for name, _kind, _shape, _true in _edge_nn_param_slots(spec)
        ]
        inputs = list(segment_tensors)
        if spec.needs_positions:
            inputs.append(positions)
        inputs.extend(parameters)
        output = _EdgeNNTileVjpFunction.apply(
            fwd_plan, vjp_plan, spec, dst_i32, src_i32, num_edges,
            graph.schema.num_dst, *inputs)
        variant = CompiledVariant(
            # Same structure as the inference variant, distinct autograd
            # contract: keep both inspectable instead of overwriting it.
            key=(*key, "edge-nn-vjp"),
            backend="cuda",
            provider=vjp_plan.result.provider.display_name(),
            lowering="gf-python-emit-edge-nn-tile-vjp",
            provider_key=vjp_plan.result.provider.cache_key(),
            domain_plan="\n".join((
                "// Tiga captured domain-plan",
                f"apply @{type(self).__qualname__}",
                "  recognize edge_nn_subgraph reducer(sum)",
                ...
                (f"  edge_tile block_e={spec.block_e} k_pad={spec.k_pad} "
                 f"out={spec.out_width}/{spec.out_pad}"),
                "  vjp symbolic-reverse recompute-tile",
            )),
            passes=(
                "capture-edge-nn-subgraph",
                "translate-fx-to-message-dag",
                "prove-edge-nn-tile-structure",
                "python-emit-edge-nn-tile-ttir",
                "symbolic-edge-nn-vjp",
                "python-emit-edge-nn-vjp-ttir",
                "provider-compile-serialized-ttir",
            ),
            artifacts={
                "ttir": emit_edge_nn_tile_ttir(spec),
                "ttir_vjp": emit_edge_nn_vjp_ttir(spec),
            },
            remarks=(
                ("Python emission backend (phase 2); the C++ "
                 "gf-kernel-to-ttir emitter replaces it in a later phase"),
                ("training path: backward recomputes per-edge activations "
                 "inside the tile; no [E, ·] tensor is materialized in "
                 "either direction"),
            ),
        )
        self._record_variant(variant)
    ...
    def _execute_edge_nn_attention(
        self, key, graph, row_ptr, col_idx, src, dst, edge, params,
        item, tensors,
    ):
        """Compile nn-scored online softmax to row-centric tile kernels.

        Phase 3 envelope: float32, CSR relation, scalar nn score, one
        field-gathered value vector, no block pruning.  Grad-mode calls take
        the fused backward (``_EdgeNNAttentionFunction``); anything outside
        the envelope returns ``None`` and the eager oracle owns the
        semantics.
        """
        if self.reducer.block_prune_threshold is not None:
            return None
        from ...compiler.edge_nn_attention import (
            build_edge_nn_attention_spec,
            emit_edge_nn_attention_ttir,
            emit_edge_nn_attention_vjp_ttir,
        )

        dag = item.score.args[0] if item.score.op == "nn_subgraph" else None
        if dag is None:
            return None
        weights = dict(dag.graph_module.named_parameters())
        if (
        ...
        position_dim = (
            int(positions.shape[1])
            if positions is not None and positions.ndim == 2 else None)
        spec = build_edge_nn_attention_spec(
            item, field_widths=field_widths, position_dim=position_dim)
        if spec is None:
            return None
        requires_tape = torch.is_grad_enabled() and (
            any(value.requires_grad for value in tensors)
            or any(parameter.requires_grad for parameter in weights.values())
            or (spec.score.needs_positions and positions.requires_grad)
        )
        if requires_tape:
            # Training path: fused forward + fused row-centric backward.
            fwd_plan = _edge_nn_attention_plan(spec)
            vjp_plan = _edge_nn_attention_vjp_plan(spec)
            parameters = [
                weights[name]
                for name, _kind, _shape, _true in _edge_nn_param_slots(
                    spec.score)
            ]
            row_ptr_i32 = row_ptr.to(torch.int32).contiguous()
            col_i32 = col_idx.to(torch.int32).contiguous()
            value_mapping = {
                "src_field": src, "dst_field": dst, "edge_field": edge,
            }[spec.value_kind]
            value_tensor = value_mapping[spec.value_name]
            if not value_tensor.is_contiguous():
                value_tensor = value_tensor.contiguous()
            segment_tensors = []
            for segment in spec.score.segments:
                if segment.kind == "displacement":
                    continue
                mapping = {
                    "src_field": src, "dst_field": dst, "edge_field": edge,
                }[segment.kind]
                field = mapping[segment.name]
                segment_tensors.append(
                    field if field.is_contiguous() else field.contiguous())
            if spec.score.needs_positions and not positions.is_contiguous():
                positions = positions.contiguous()
            output = _EdgeNNAttentionFunction.apply(
                fwd_plan, vjp_plan, spec, row_ptr_i32, col_i32,
                graph.schema.num_dst,
                *segment_tensors,
                *([positions] if spec.score.needs_positions else []),
                *parameters, value_tensor)
            variant = CompiledVariant(
                key=(*key, "edge-nn-attention-vjp"),
                backend="cuda",
                provider=vjp_plan.result.provider.display_name(),
                ...
                domain_plan="\n".join((
                    "// Tiga captured domain-plan",
                    f"apply @{type(self).__qualname__}",
                    "  recognize edge_nn_subgraph reducer(online_softmax)",
                    (f"  row_tile block_n={spec.score.block_e} "
                     f"k_pad={spec.score.k_pad} value={spec.value_width}/"
                     f"{spec.value_pad}"),
                    "  vjp online-softmax-jacobian symbolic-chain-reverse",
                )),
                passes=(
                    "capture-edge-nn-subgraph",
                    "translate-fx-to-message-dag",
                    "prove-edge-nn-attention-structure",
                    "python-emit-edge-nn-attention-ttir",
                    "symbolic-edge-nn-attention-vjp",
                    "python-emit-edge-nn-attention-vjp-ttir",
                    "provider-compile-serialized-ttir",
                ),
                artifacts={
                    "ttir": emit_edge_nn_attention_ttir(spec),
                    "ttir_vjp": emit_edge_nn_attention_vjp_ttir(spec),
                },
                remarks=(
                    ("training path: forward persists only the O(N) softmax "
                     "state (m, l); backward replays the score chain per "
                     "row chunk — no [E, ·] tensor in either direction"),
                ),
            )
            self._record_variant(variant)
            return output
        ...
        score_spec = spec.score
        segment_tensors = []
        for segment in score_spec.segments:
            if segment.kind == "displacement":
                continue
            mapping = {
                "src_field": src, "dst_field": dst, "edge_field": edge,
            ...
            segment_tensors.append(
                value if value.is_contiguous() else value.contiguous())
        if score_spec.needs_positions and not positions.is_contiguous():
            positions = positions.contiguous()
        padded = []
        for name, kind, shape, _true in _edge_nn_param_slots(score_spec):
            if kind == "matrix":
                padded.append(_EDGE_NN_WEIGHT_CACHE.weight(
                    weights[name], *shape))
            else:
        ...
        row_ptr_i32 = row_ptr.to(torch.int32).contiguous()
        col_i32 = col_idx.to(torch.int32).contiguous()
        plan = _edge_nn_attention_plan(spec)
        launch_args = [row_ptr_i32, col_i32, *segment_tensors]
        if score_spec.needs_positions:
            launch_args.append(positions)
        launch_args.extend(padded)
        launch_args.append(value_tensor)
        num_rows = graph.schema.num_dst
        output, _m, _l = plan.run(tuple(launch_args), num_rows=num_rows)

        variant = CompiledVariant(
            key=key,
            backend="cuda",
            provider=plan.result.provider.display_name(),
            lowering="gf-python-emit-edge-nn-attention-tile",
            provider_key=plan.result.provider.cache_key(),
            domain_plan="\n".join((
                "// Tiga captured domain-plan",
                f"apply @{type(self).__qualname__}",
                "  recognize edge_nn_subgraph reducer(online_softmax)",
                f"  segments {[f'{s.kind}:{s.name}x{s.width}' for s in score_spec.segments]}",
                f"  layers {[type(layer).__name__ for layer in score_spec.layers]}",
                f"  value {spec.value_kind}:{spec.value_name}x{spec.value_width}",
                (f"  row_tile block_n={score_spec.block_e} "
                 f"k_pad={score_spec.k_pad} value={spec.value_width}/"
                 f"{spec.value_pad}"),
                "  online-softmax scf.for iter_args (m, l, acc)",
            )),
            passes=(
                "capture-edge-nn-subgraph",
                "translate-fx-to-message-dag",
            ...
            artifacts={"ttir": emit_edge_nn_attention_ttir(spec)},
            remarks=(
                ("Python emission backend (phase 3); row-centric online "
                 "softmax with nn-produced scores, no O(E) score or "
                 "attention-weight tensor is materialized"),
                ("inference path; calls that could request gradients take "
                 "the fused row-centric VJP "
                 "(gf-python-emit-edge-nn-attention-vjp)"),
            ),
        )
        self._record_variant(variant)
        return output
    ...
    def _evaluate_reference(
        ...
        if isinstance(self.reducer, OnlineSoftmaxReducer):
            ...
            from ...reducer import ReducerCall

            if isinstance(message, ReducerCall):
                # A staged self.reducer(...) binding carries the message(s);
                # the torch oracle evaluates single-message bindings only.
                if message.reducer is not self.reducer:
                    raise ValueError(
                        "edge() returned a binding for a different reducer")
    ...
    def _make_reference_variant(self, key, graph, src, dst, edge):
        ...
        plan = "\n".join(
            [
                "// Tiga domain-plan (not yet parsed MLIR)",
