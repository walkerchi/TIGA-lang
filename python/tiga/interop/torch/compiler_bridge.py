"""Torch interoperability around the canonical Tiga MLIR compiler.
...
``gf-translate`` provider boundary, where it isolates Tiga's pinned MLIR
    ...
    *,
    ...
    if tool is None:
        raise FileNotFoundError(
            "gf-translate was not found; set TIGA_TRANSLATE or pass "
            "gf_translate explicitly"
        )
    with TemporaryDirectory(prefix="tiga-kernel-") as directory:
...
def parse_kernel_ttir_plan(module: str) -> KernelTTIRPlan:
    ...
    match = re.match(
        r"// tiga\.launch entry=(\S+) block_rows=(\d+) "
        r"num_warps=(\d+) abi=([A-Za-z0-9_,]+)\n",
        module,
    )
    if match is None:
        raise RuntimeError("gf-translate output has no Tiga launch manifest")
...
def lower_mlir_stages(
    ...
    *,
    ...
    if tool is None:
        raise FileNotFoundError(
            "gf-opt was not found; set TIGA_OPT or pass gf_opt explicitly"
        )
    ...
    with TemporaryDirectory(prefix="tiga-mlir-") as directory:
