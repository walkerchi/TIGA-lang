...
def capture_edge_staged(method, params: Mapping[str, Any]):
    """``capture_edge`` that also keeps staged reducer items.

    ``OnlineSoftmaxReducer.__call__`` returns an ``OnlineSoftmaxItem``
    (score/value Expr pair), not an Expr; attention lowerings need the pair.
    """
    from ..reducer.core import OnlineSoftmaxItem  # lazy: reducer imports us
    ...
    try:
        ...
        return None
    return result if isinstance(result, (Expr, OnlineSoftmaxItem)) else None
