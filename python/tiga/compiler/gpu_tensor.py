"""CUDA provider for canonical Tiga Tensor IR.

The stable input is ``gf_tensor`` MLIR.  A separate Tiga translator
emits serialized TTIR, then the active vendor Triton distribution lowers that
provider IR to target assembly. Native Tiga buffers launch through the
...
class GPUExecutable:
    ...
    def artifact(self) -> Path:
        # Triton owns a versioned persistent cache rather than one Tiga
    ...
    def prepare(self, output: Tensor):
        ...
        if self.uses_torch_storage and all_external_storage:
            ...
            # the standalone Tiga-Buffer path below.
...
def _physicalize_storage(
    ...
    budget_text = os.environ.get("TIGA_CHECKPOINT_BUDGET_BYTES")
    spill_budget_text = os.environ.get(
        "TIGA_CHECKPOINT_SPILL_BUDGET_BYTES"
    )
    try:
        memory_budget_bytes = -1 if budget_text is None else int(budget_text)
    except ValueError as error:
        raise ValueError(
            "TIGA_CHECKPOINT_BUDGET_BYTES must be -1 or non-negative"
        ) from error
    ...
    try:
        ...
        raise ValueError(
            "TIGA_CHECKPOINT_SPILL_BUDGET_BYTES must be -1 or non-negative"
        ) from error
...
def _find_translate() -> Path:
    ...
    if executable is None:
        raise RuntimeError(
            "gf-translate is unavailable; set TIGA_TRANSLATE or install "
            "a native Tiga wheel"
        )
...
def _translate(module: str) -> tuple[str, str, int, int, int]:
    ...
    match = re.match(
        r"// tiga\.tensor entry=(\S+) block_rows=(\d+) "
...
def compile_tensor(
    ...
    *,
    ...
    matmul_provider = os.environ.get(
        "TIGA_MATMUL_PROVIDER", "auto").lower()
    if matmul_provider not in {"auto", "library", "ttir"}:
        raise ValueError(
            "TIGA_MATMUL_PROVIDER must be auto, library, or ttir")
        ...
        executable = GPULibraryExecutable(
            ...
            semantic_hash, compile_ms, False,
            {
                ...
                "library_dispatch": (
                    ...
                    "FP32 accumulation; current Tiga/Torch stream"
                ),
            },
