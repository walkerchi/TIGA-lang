...
| K0 | PARTIAL | exact procedural kNN build+consume | `Graph.knn` 的精确语义、位置 mutation differential、固定 row metadata 与 compiler-generated consume TTIR rebind 已通过；但 N=8192/D3/k32 仍由 exhaustive Euclidean cdist/top-k 构图，Prepared 3.9072 ms 对 matched pipeline 3.9137 ms 只有 1.0017x（CI low 1.0005），只能算 parity。关闭本项还需要 provider-neutral ranked-relation IR、candidate-tile local top-k + hierarchical merge、memory-budgeted task bundle、selected-edge consume fusion，以及至少两个 N/D/k bucket 的端到端严格 gate；缓存 cell directory 的 reuse 时间不得冒充完整 rebuild |
