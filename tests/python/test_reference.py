...
class RadiusDisplacement(tg.MessagePassing):
    reducer = tg.sum()
...
class DiffusionFlux(tg.MessagePassing):
    reducer = tg.sum()
...
class TilePrunedDenseAttention(tg.MessagePassing):
    reducer = tg.online_softmax(block_prune_threshold=0.125)
...
class DifferentTypeRadius(tg.RadiusGraph):
...
class ReferenceTest(unittest.TestCase):
    ...
    def test_compiler_derived_source_vjp_reuses_transpose_relation(self):
        ...
        graph = tg.Graph.from_csr(row_ptr, col_idx, num_src=8)
    ...
    def test_degree_statistics_participate_in_planning_key(self):
        ...
        def graph_from_degrees(degrees):
            ...
            return tg.Graph.from_csr(row_ptr, col_idx, num_src=4)
    ...
    def test_source_index_span_refines_physical_planning_key(self):
        ...
        local_graph = tg.Graph.from_csr(row_ptr, local, num_src=nodes)
        remote_graph = tg.Graph.from_csr(row_ptr, remote, num_src=nodes)
    ...
    def test_graph_halo_is_declarative_and_preserves_graph_type(self):
        row_ptr, col_idx = ring_csr(8, 2)
        graph = tg.Graph.from_csr(row_ptr, col_idx, num_src=8)
        mesh = tg.DeviceMesh("cuda", (2, 4), names=("x", "y"))

        distributed = graph.halo(
            mesh,
            partition=tg.ByDestination(mesh_axis="x", balance="edges"),
            depth="auto",
        )

        self.assertIs(type(distributed), tg.Graph)
    ...
    def test_graph_halo_requires_an_active_distributed_runtime(self):
        row_ptr, col_idx = ring_csr(8, 2)
        graph = tg.Graph.from_csr(row_ptr, col_idx, num_src=8).halo(
            tg.DeviceMesh("cpu", 2), depth=1)
        x = torch.randn(8)
    ...
    def test_graph_halo_validates_mesh_axis_and_depth(self):
        graph = tg.Graph.dense(4)
        mesh = tg.DeviceMesh("cuda", (2, 2), names=("x", "y"))
        with self.assertRaisesRegex(ValueError, "outside"):
            graph.halo(mesh, partition=tg.ByDestination(mesh_axis=2))
    ...
    def test_dense_graph_normalizes_unindexed_cuda_device(self):
        graph = tg.Graph.dense(4, device="cuda")
        self.assertEqual(
            graph.device,
            tg.Device.parse(f"cuda:{torch.cuda.current_device()}"),
        )
    ...
    def test_dense_structured_reducer_uses_direct_compiler_ttir(self):
        ...
        graph = tg.Graph.dense(nodes, device=query.device)
    ...
    def test_dense_graph_is_implicit_and_attention_uses_same_semantics(self):
        ...
        graph = tg.Graph.dense(sources, destinations)
    ...
    def test_ndata_binds_both_endpoint_roles_on_homogeneous_graphs(self):
        ...
        graph = tg.Graph.from_csr(row_ptr, col_idx, num_src=4)
    ...
    def test_large_dense_graph_refuses_csr_materialization(self):
        graph = tg.Graph.dense(4096, device="cpu")
    ...
    def test_dynamic_radius_implicit_geometry(self):
        ...
        graph = tg.Graph.radius(positions, 0.75)
    ...
    def test_dynamic_radius_reuses_equivalent_physical_snapshot(self):
        ...
        first_graph = tg.Graph.radius(positions, 0.75)
        kernel = RadiusDistanceAggregation()
        first = kernel(
            graph=first_graph, src={"x": x}, dst={"x": x})

        equivalent_graph = tg.Graph.radius(positions, 0.75)
        ...
        positions.add_(2.0)
        changed_graph = tg.Graph.radius(positions, 0.75)
        changed = kernel(
    ...
    def test_generated_cell_directory_reuses_immutable_snapshot_and_invalidates(self):
        ...
        graph = tg.Graph.radius(positions, 0.25)
    ...
    def test_linear_weighted_sum_dispatches_native_sparse_on_cpu(self):
        ...
        self.assertIsInstance(finding, tg.AnalysisFinding)
        ...
        with self.assertRaisesRegex(ValueError, "disposition"):
            tg.AnalysisFinding("planning", "maybe", "invalid")
    ...
    def test_static_auto_warm_call_uses_guarded_executable(self):
        ...
        self.assertIn("variant cache: hits=1, misses=1", kernel.explain())
    ...
    def test_exact_knn_is_procedural_and_materializes_matched_neighbors(self):
        ...
        graph = tg.Graph.knn(positions, 2)
    ...
    def test_bipartite_knn_and_transpose_form_clustering_relations(self):
        ...
        graph = tg.Graph.knn(queries, 1, candidates=candidates)
